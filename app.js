...
let players=uniquePlayers(PLAYER_SEED);
...
function getCandidates(pos){
  return players.filter(p=>p.pos===pos.n).sort((a,b)=>a.name.localeCompare(b.name));
}
...
function renderPositions(){
  ...
  $$('.position-player-search').forEach(inp=>inp.addEventListener('input',()=>{
    const q=inp.value.toLowerCase().trim(); const pos=inp.dataset.pos;
    $$('#positions .position-player-list[data-pos="'+pos+'"] .player-option').forEach(row=>{row.style.display=(!q || row.textContent.toLowerCase().includes(q))?'flex':'none';});
  }));
...
function progress(){const complete=POSITIONS.filter(p=>selectedFor(p).length===10).length;$('#progressText').textContent=`${complete} / 15 posisies`;$('#progressBar').style.width=`${complete/15*100}%`;$('#submitVote').disabled=complete!==15}
async function submit(){
  const complete=POSITIONS.every(p=>selectedFor(p).length===10);
  if(!complete){$('#voteMessage').textContent='Kies 10 spelers en rangskik hulle #1–#10 vir al 15 posisies.';return;}
...
function renderPlayers(){
  const q=($('#playerSearch')?.value||'').toLowerCase().trim();const pos=$('#playerFilter')?.value||'all';
  const list=directoryPlayers.filter(p=>(pos==='all'||p.pos===pos)&&(!q||p.name.toLowerCase().includes(q)));
...
const HISTORICAL_PLAYER_SOURCES=[
  ['South Africa','🇿🇦','SouthAfrica.csv'],['New Zealand','🇳🇿','NewZealand.csv'],['Australia','🇦🇺','Australia.csv'],
  ['England','🏴ENG','England.csv'],['Wales','🏴WAL','Wales.csv'],['Ireland','🇮🇪','Ireland.csv'],['Scotland','🏴SCO','Scotland.csv'],
  ['France','🇫🇷','France.csv'],['Italy','🇮🇹','Italy.csv'],['Argentina','🇦🇷','Argentina.csv'],['Fiji','🇫🇯','Fiji.csv'],['Japan','🇯🇵','Japan.csv'],['Tonga','🇹🇴','Tonga.csv'],['Samoa','🇼🇸','Samoa.csv'],['Canada','🇨🇦','Canada.csv'],['USA','🇺🇸','USA.csv']
];
const HISTORICAL_PLAYER_BASE='https://raw.githubusercontent.com/hautahi/Rugby-Wanderers/master/data/';
...
async function loadHistoricalPlayerDirectory(){
  ...
  await Promise.all(HISTORICAL_PLAYER_SOURCES.map(async ([country,flag,file])=>{
    try{
      const r=await fetch(HISTORICAL_PLAYER_BASE+file,{cache:'force-cache'});if(!r.ok)return;
      const text=await r.text();const lines=text.split(/\r?\n/).filter(Boolean);if(lines.length<2)return;
      const head=csvFields(lines[0]).map(x=>x.toLowerCase());const ni=head.indexOf('name'),di=head.indexOf('debut'),pi=head.indexOf('position');
      for(const line of lines.slice(1)){
        const row=csvFields(line),name=(row[ni]||'').trim(),year=debutYear(row[di]);if(!name||year<1978||year>2026)continue;
        const pos=positionNumber(row[pi]);merged.push({name,country:flag,pos});
      }
    }catch(e){}
  }));
  directoryPlayers=uniquePlayers(merged);
  renderPlayers();
}
...
async function predictorCreateJoin(mode){const {name}=setPredictorIdentity();if(!name){$('#predictorStatus').textContent='Voer jou naam in.';return}if(API_URL){try{const r=await fetch(API_URL.replace(/\/$/,'')+'/predictor/pools',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:mode,nickname:name,deviceId:predictorDevice(),code:$('#predictorPool').value.trim().toUpperCase()})});const d=await r.json();if(!r.ok)throw new Error(d.message||'Pool kon nie verwerk word nie.');localStorage.setItem('lekkerRugbyPredictorPool',d.code);$('#predictorPool').value=d.code;$('#predictorStatus').innerHTML='✓ Pool: <span class="pool-code">'+esc(d.code)+'</span>';await renderPredictor();return}catch(e){$('#predictorStatus').textContent='⚠️ '+e.message;return}}let code=$('#predictorPool').value.trim().toUpperCase();if(mode==='create'||!code){code='LR'+Math.random().toString(36).slice(2,7).toUpperCase();}localStorage.setItem('lekkerRugbyPredictorPool',code);$('#predictorPool').value=code;$('#predictorStatus').innerHTML='✓ Plaaslike pool: <span class="pool-code">'+esc(code)+'</span> (sentraal beskikbaar wanneer Worker gekoppel is).';await renderPredictor()}

const TEAM_FIVE_YEAR_OVERVIEW={
  'South Africa':{note:'2022–2026 internasionale rekord en kompetisie-argief. 2026 word volledig in die Springbok-profiel vertoon.',source:'https://www.espn.com/rugby/results/_/team/5/',years:['2022','2023','2024','2025','2026']},
  'New Zealand':{note:'2022–2026 internasionale rekord-argief, insluitend Rugby Championship, Wêreldbeker en Greatest Rivalry.',source:'https://www.espn.com/rugby/results/_/team/8/',years:['2022','2023','2024','2025','2026']},
  'Ireland':{note:'2022–2026 internasionale rekord-argief, insluitend Six Nations en toetse.',source:'https://www.espn.com/rugby/results/_/team/3/',years:['2022','2023','2024','2025','2026']},
  'France':{note:'2022–2026 internasionale rekord-argief, insluitend Six Nations en Wêreldbeker.',source:'https://www.espn.com/rugby/results/_/team/9/',years:['2022','2023','2024','2025','2026']},
  'England':{note:'2022–2026 internasionale rekord-argief, insluitend Six Nations en toetse.',source:'https://www.espn.com/rugby/results/_/team/1/',years:['2022','2023','2024','2025','2026']},
  'Scotland':{note:'2022–2026 internasionale rekord-argief, insluitend Six Nations en toetse.',source:'https://www.espn.com/rugby/results/_/team/2/',years:['2022','2023','2024','2025','2026']},
  'Australia':{note:'2022–2026 internasionale rekord-argief, insluitend Rugby Championship en toetse.',source:'https://www.espn.com/rugby/results/_/team/6/',years:['2022','2023','2024','2025','2026']},
  'Argentina':{note:'2022–2026 internasionale rekord-argief, insluitend Rugby Championship en toetse.',source:'https://www.espn.com/rugby/results/_/team/10/',years:['2022','2023','2024','2025','2026']},
  'Fiji':{note:'2022–2026 internasionale rekord-argief, insluitend Pacific rugby en toetse.',source:'https://www.espn.com/rugby/results/_/team/7/',years:['2022','2023','2024','2025','2026']},
  'Wales':{note:'2022–2026 internasionale rekord-argief, insluitend Six Nations en toetse.',source:'https://www.espn.com/rugby/results/_/team/4/',years:['2022','2023','2024','2025','2026']}
};
function renderNews(){
...
function startLiveStats(){clearInterval(liveTimer);loadLiveStats();liveTimer=setInterval(()=>{if(document.getElementById('stats')?.classList.contains('active'))loadLiveStats()},60000)}
...
loadLocalPlayers();loadVoteCounts().then(()=>renderPositions());progress();renderRankings();renderPlayers();renderAdmin();renderTeamStats();renderNews();loadHistoricalPlayerDirectory().catch(()=>{});
